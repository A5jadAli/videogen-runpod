"""
VideoGen - Wan 2.2 A14B Image-to-Video on Modal
===============================================
Converted from RunPod serverless to Modal.

Architecture (same async pattern as the trainer):
  - Web endpoint (CPU) validates + spawns a background GPU job, returns immediately
  - Background GPU function (H200) runs Wan 2.2 generation + 4-stage post-processing
  - Webhook fires on completion/failure — client never needs to stay connected
  - Cancel endpoint for individual jobs

Setup:
  modal run app.py::download_models   # ONE-TIME: download Wan 2.2 (~126GB) to volume
  modal deploy app.py
"""
...
def run_generation(data: dict):
    ...
    try:
        ...
        print(f"GPU: {gpu_info.get('gpu')}, VRAM total: {gpu_info.get('vram_total_gb')}GB")
        ...
        traceback.print_exc()
        webhook_response(
            job.webhook_url, False, 500,
            f"Video generation failed: {str(e)}", {"job_id": job.job_id}
        )
...
def generate(data: dict) -> dict:
    ...
    Validates input, spawns a background GPU job, returns job_id + call_id.
    Webhook fires when the video is ready (or on failure).
    """
    if not data:
...
@app.function(timeout=60)
@modal.fastapi_endpoint(method="POST", docs=True)
def cancel_generation(data: dict) -> dict:
    """Cancel a specific video job by its call_id."""
